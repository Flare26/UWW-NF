...
public class CameraController_withBounds : MonoBehaviour
{
    ...
    void Start()
    {
        if (leftBounds == null || rightBounds == null || upperBounds == null ||
lowerBounds == null)
        {
            Debug.Log("bounds objects missing on Camera Controller on " +
gameObject.name);
        }
    }
    void FixedUpdate()
    {
        if (target != null)
        {
            ...
            transform.position = new Vector3(transform.position.x + differance.x *
smoothingTimePercentage, transform.position.y + differance.y *
smoothingTimePercentage, -10f);
            float newX;
