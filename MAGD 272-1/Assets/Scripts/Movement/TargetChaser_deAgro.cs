...
public class TargetChaser_deAgro : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            target =
GameObject.FindObjectOfType<PlayerInputController>().gameObject;
        }
    ...
    void FixedUpdate()
    {
        ...
        if (!isAgro && (target.transform.position - transform.position).magnitude
<= agroDistance)
        {
            isAgro = true;
        }
...
transform.position).sqrMagnitude <= stoppingDistance * stoppingDistance)
                ...
                else
                {
                    ...
                    motor.Move(new Vector2(target.transform.position.x,
